...
public class CoverEvaluator implements FitnessEvaluator<List<Integer>> {
                             ...
                             List<? extends List<Integer>> population)
    {
        ...
        double camerasScore = 1/(double)candidate.size() * numOfCamerasWeight; // as smaller the number of cameras, the bigger the score
        double coverScore = (double)edgesInCover/graph.edges.size() * (1-numOfCamerasWeight); // as bigger the number of edge in cover, the bigger the score
